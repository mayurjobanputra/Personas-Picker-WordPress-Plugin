...
function persona_tabs_shortcode($atts) {
    ...
    while ($query->have_posts()) {
        ...
        //ensure line breaks in the content are displayed as <br> tags
        $post_content = wpautop($post_content);
        //output the tab-pane with the id of the sanitized title and the title and content of the persona
        //jquery to hide all tab-panes initially and then when someone clicks on a persona title generated by the persona_picker_shortcode, it will show the content of that persona by using display: flex for that specific persona generated by this shortcode.
        //add inline style to the div to display: none so that it is hidden initially
        $output .= "<div class='tab-pane' id='tab-" . $slug_title . "'>" . $post_content . "</div>";
    ...
    //some typewriter styling to the persona content so that when it appears, it appears as if it is being typed out
    $output .= "<style>.tab-pane{border-right: .15em solid #666;white-space: nowrap;overflow: hidden;animation: typing 3.5s steps(40, end), blink-caret .75s step-end infinite;}</style>";
